...
ReadingProcess::ReadingProcess()
{
    ip = "192.168.0.10";
    port = 1235;
    readSck = new QTcpSocket();
    readSck->connectToHost(ip,port);
    QObject::connect(readSck, SIGNAL(readyRead()), this, SLOT(readingLoop()));
    QObject::connect(readSck, SIGNAL(error(QAbstractSocket::SocketError)),
            this, SLOT(displayError(QAbstractSocket::SocketError)));
}

void ReadingProcess::readingLoop()
{
    qDebug()<<"reading connected";
}

void ReadingProcess::displayError(QAbstractSocket::SocketError err)
{
    QMessageBox warn;
    QString msg;
